...
def main(fitness_function=sum_fitness,
         ...
         stop_fname='.stop',
         params_fname='.__current_best_average_worst__.csv'):
    """
    run the ga
    """
...
def setupGAEngine(genome=G1DList.G1DList(),
    ...
    ga.stepCallback.add(lambda x: save_best_average_worst(x, params_fname))
...
#todo: to view current measurement. must see if it won't slow down (maybe not)
...
def get_population(ga_engine=GSimpleGA.GSimpleGA(G1DList.G1DList())):
    pop = ga_engine.getPopulation()

    # extract the population and fitnesses into numpy arrays:
    np_pop = []
    fitnesses = []

    for ind in pop:
        np_pop.append(np.copy(ind.genomeList))
        fitnesses.append(ind.fitness)

    np_pop = np.array(np_pop)
    fitnesses = np.array(fitnesses)

    return np_pop, fitnesses

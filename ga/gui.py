...
#                              "index of\nprojection\nto plot")
        ...
        self.best_average_worst_plot = \
            MyMplCanvas(page, width=5, height=10, dpi=100)
        # create_widget("matplotlib canvas")
        baw_nav = MplCanvasWithNav(self.best_average_worst_plot)
        baw = labeled_widget(baw_nav, "best, average, worst parameters")
        ...
        try:
            ...
            if os.path.exists(fname):
                ...
                if np.size(shape) == 2 and shape != self.fitness_history_shape:
                    # todo: clear data or append to existing data
                    # self.fitness_history_plot.axes.
                    self.fitness_history_plot.axes.plot(data)
                    self.fitness_history_plot.axes.legend(cols,
                                                          loc='best')
                    self.fitness_history_plot.axes.set_xlabel('generation')
                    self.fitness_history_plot.draw()
                    self.fitness_history_shape = shape

            # get the current generation's best average worst parameters:
            fname = os.path.join(ga_folder(), '.__current_best_average_worst__.csv')
            df = pd.read_csv(fname)
            best = df['best'].values
            average = df['average'].values
            worst = df['worst'].values

            if not hasattr(self.best_average_worst_plot, 'ax1'):
                fig = self.best_average_worst_plot.fig
                fig.clear()
            ...
            self.ax1.cla()
            self.ax1.plot(best)
            self.ax1.set_ylabel('best')
            self.ax1.set_xticks([])

            self.ax2.cla()
            self.ax2.plot(average)
            self.ax2.set_ylabel('average')
            self.ax2.set_xticks([])

            self.ax3.cla()
            self.ax3.plot(worst)
            self.ax3.set_ylabel('worst')
            self.ax3.set_xlabel('parameter index')

            self.best_average_worst_plot.fig.tight_layout()

            self.best_average_worst_plot.draw()

...
def calculate_hessian(mu1_plus_hat=[[[]]],
    ...
    hpp_t, hpp_delta_t =\
        calculate_Hplusplus_t(mu1_plus_hat, mu2_plus_hat, vec_i, vec_p)
    ...
    hpp_delta, __, __ = \
        Hplusplus_delta2freq(hpp_delta_t, times, wo, wavelengths)
    hpm, __, __ =\
    ...
    H += (np.reshape(E, (N, 1))*E) * (hpp + hpp_delta)
    ...
    if graph:
        plot_hessian(H, wavs = wavs, figname=figname)
...
def calculate_Hplusplus_t(mu1_plus_hat=[[[]]],
    ...
    h_plus_plus_delta_vec = - 1j / hbar * (d1 - d2)
    # diag_vec = 1j / hbar * (d1 + d2)
    # h_plus_plus -= np.diag(diag_vec.ravel())

    # imposing the t>t' condition:
    h_plus_plus = np.triu(h_plus_plus)
    # h_plus_plus = np.tril(h_plus_plus)
    ...
    if graph:
        ...
        title = 'hplusplus_delta vs. time index'
        plt.figure(title)
        plt.plot(np.real(h_plus_plus_delta_vec))
        plt.twinx()
        plt.plot(np.imag(h_plus_plus_delta_vec), 'g')
        plt.title(title)

    return h_plus_plus, h_plus_plus_delta_vec
...
def calculate_Hplusminus_t(mu1_plus_hat=[[[]]],
    ...
    # imposing the t>t' condition:
    # h_plus_minus = np.tril(h_plus_minus)
...
def Hplusplus2freq(hpp_t=[[]], times=[], wo=0,
                   wavelengths=[],
                   graph=False,
                   figname='h_plus_plus_freq vs. wavelengths'):
    """
    ...
    if graph:
        title = 'h plus plus freq vs. wavelengths'
...
#todo: new function to transform term split from Hpp(t) that has the delta func
# with ko the index corresponding to wo, construct a meshgrid m=x+y with
# x and y equal to indices-ko. then construct the transform of the new
# term as F(ko + m). might have to pad F with zeros equal to half it's
# length on each side.
def Hplusplus_delta2freq(hpp_delta_t=[], times=[], wo=0,
                         wavelengths=[], graph=False,
                         figname='h_plus_plus_delta vs. wavelengths'):
    """
    If wavelengths=[] will return h_freq vs. freqs.
    :param hpp_t:
    :param times:
    ...
    :return:
    """
    # transform hpp_delta_t to w:
    dt = times[1] - times[0]#;plt.figure('ht');plt.plot(times, np.real(hpp_delta_t), '.-');plt.xlim((-200, 200))
    hw, w = ft2Fw(np.ravel(hpp_delta_t), dt, wo)#;plt.figure('hw');plt.plot(w, np.real(hw), '.-');plt.xlim(((2.34, 2.37)))

    # find the index in hpp_delta_t corresponding to wo (or closest)
    nt = np.size(times)
    # w = wo + 2 * np.pi * np.fft.fftfreq(nt, dt)
    # w = np.fft.fftshift(w)
    wo_index = np.argmin(np.abs(w - wo))#;print wo_index

    # generate a meshgrid corresponding to (w1-wo) + (w2-wo) indices
    indices = np.arange(nt) - wo_index#; plt.figure('ind');plt.plot(indices)
    xx, yy = np.meshgrid(indices, indices)
    meshgrid = xx + yy#;plt.figure('mesh');plt.pcolormesh(meshgrid);plt.colorbar()

    # evaluate hpp_delta_t at the meshgrid + wo_index
    meshgrid += wo_index
    hwpadded = np.append(np.zeros(nt), hw.ravel())
    hwpadded = np.append(hwpadded, (np.zeros(nt)))
    h_freq = hwpadded[nt + meshgrid]#;plt.figure('ht2d');plt.pcolormesh(np.real(ht2d));plt.colorbar()
    freqs = w / (2 * np.pi)
    c = constants.c * 1e9 / 1e15  # nm/fs
    wavs = c / np.copy(freqs)
    ...
    if graph:
        title = 'h_plus_plus_delta vs. wavelengths'
        plot_hessian(hessian_wav=h_freq, wavs=wavs,
                     figname=figname)
        plt.title(title)
...
def Hplusminus2freq(hpm_t=[[]], times=[], wo=0,
    ...
    # h_flipped = 0.5 * (h_flipped[...] + h_flipped.T[...])
    h_freq, freqs, wavs = Mt2Mfreq(h_flipped, times, wo)
...
def bwg_2d_plot(matrix, x=[], y=[],
    ...
    one_to_zero = np.linspace(1, 0, nplus) ** exponent
    m = np.vstack((one_to_zero, np.ones(nplus), one_to_zero))
    colors += [tuple(row) for row in m.T]
...
def interpolate_complex(xnew, xold, yold,
                        interpolation='smooth'):
    yold_re = np.real(yold)
    yold_im = np.imag(yold)

    ynew_re = interpolate(xnew, xold, yold_re,
                          interpolation)
    ynew_im = interpolate(xnew, xold, yold_im,
                          interpolation)

    return ynew_re + 1j * ynew_im
    # amps = np.abs(old_y)
...
def Mt2Mfreq(Mt, times, wo):
    ...
    """
    assert (len(np.shape(Mt)) == 2)
    assert (np.shape(Mt)[0] == np.shape(Mt)[1])

    # Mf = np.fft.fft2(np.fft.fftshift(Mt))
